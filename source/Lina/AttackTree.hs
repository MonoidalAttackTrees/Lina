...
showANode i id node labels attributes lt rt = 
  case B.lookupR id labels of
    Just l -> node++"("++(show id)++","++(show l)++")\n"
                       ++ tabs++"("++lt++")\n"
                       ++ tabs++"("++rt++")"
    Nothing -> show $ LabelNotFound "showANode" id
 where
   tabs = take i $ repeat '\t'
...
showIAttack' i labels attributes (ANDA id lt rt) = showANode i id "AND" labels attributes (showIAttack' (i+1) labels attributes lt) (showIAttack' (i+1) labels attributes rt)
showIAttack' i labels attributes (SEQA id lt rt) = showANode i id "SEQ" labels attributes (showIAttack' (i+1) labels attributes lt) (showIAttack' (i+1) labels attributes rt)
...
-- min_attack :: Ord attribute => [IAttack attribute label] -> [IAttack attribute label]
-- min_attack [] = []
-- min_attack ps@(p:_) = min_attack' (get_pattribute p) ps
--  where   
--    min_attack' :: Ord attribute => attribute -> [IAttack attribute label] -> [IAttack attribute label]
